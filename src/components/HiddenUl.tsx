    ...
    return (
        <ul className='hiddenUI'>
            <button className='hiddenUI__button' onClick={toggleClass}>
                <h2 className='hiddenUI__h2'>Portfolio</h2>
            </button>

            <div className={isActive ? 'hiddenUI__list-container--open hiddenUI__list-container': 'hiddenUI__list-container'}>
                {([...Array(inputObj.value.length)].map((e, i: number) => <HiddenUlLi key={i} name={inputObj.value[i].name} href={inputObj.value[i].href} />))}
            </div>
        </ul>
    )
}
